...
import { contactList,clearMessage, mainTabChange,setUpdateMessagesList, setDetailsBroadcastList, setMyProfile, getBlockContacts,RemoveGroupsFromRooms, savedMessages,SetMessagesDelivered,  SetFilterChatMessagesList, deleteMessage, newMessageSend, setMessages, getGroupsList, getSelectRoomsDetails, getMessageList, SetUpdateGroupListFilter, SetMembersListSeenMessages, CallLogoutUser, setRegisterdDevice, setAddGroupCreated, setGoBackFromSelect, setAddChatInMessageList, SetUpdateMessagesSeenDelivered, setAddBroadCastGroup, setSelectedBroadCastList, setAddMessagesListBroadcast, removeSaveMessageList, setUserTypeing, setSaveChatList, setUpdateContactList, UpdateMediaFiles, SetMessageSeenByUser, setGroupCreatedUser, updateBroadcastGroupList, SetBroadcastMessagesUpdate, SetDetailsList, setUsersDetailsList, selectRoom, setStoreGetChatsHistory, deleteStoreGetChatsHistory, setReplaySaveChatMessages, SetUpdateChatMessagesList, setStoreUserTypes, setAddSystemGroup } from "../../redux/actions";
...
export const Ws_Omessage = ({evt, ws}) =>{
    return(async (dispatch, getState) => {
        ...
        const { Rooms, SavedMessages, MessageList, access_token, BroadcastList, scroll_chat_id, UserDetails, MainTabsSelected, DetailsList,  Contacts, SelectedRoom, userLogin } = getState()?.allReducers;

        // message scroll down fucntion
        const MsgScrollDown = (time) => {
            if(scroll_chat_id?.to_id !== location_id){
                setTimeout(() => {
                    const element = document.getElementById("chatPanelScroll");
                        ...
                        if(item?.chat){
                            ...
                            if(item?.chat_type === "broadcast"){
                                if(BroadcastList[item?.chat?.broadcast_group_id]){
                                    const broadcastData = BroadcastList[item?.chat?.broadcast_group_id];
                                    users = [{
                                        ...broadcastData,
